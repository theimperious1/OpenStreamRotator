...
from stream_managers.twitch_updater import TwitchUpdater
from stream_managers.kick_updater import KickUpdater
...
class PlatformManager:
    ...
    def update_stream_info_all(self, title: str, category: Optional[str] = None) -> dict[str, bool]:
        """Update both title and category on all enabled platforms."""
        results = {}
        for platform in self.platforms:
            results[platform.platform_name] = platform.update_stream_info(title, category)
        return results
