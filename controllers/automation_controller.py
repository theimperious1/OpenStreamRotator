        ...
        if is_live and self.last_stream_status != "live":
            ...
            if self.obs_controller:
                ...
                if current_scene != SCENE_STREAM:
                    ...
                    if self._prepared_rotation_active:
                        # A prepared rotation was running when the streamer
                        # went live.  Resume from the prepared folder — don't
                        # repoint VLC at live/.  The playback monitor and VLC
                        # source already reference the prepared folder, so
                        # just refresh the playlist from disk in case files
                        # changed while paused.
                        prep_folder = (
                            self.playback_monitor.video_folder
                            if self.playback_monitor
                            else self.config_manager.video_folder
                        )
                        success, _ = self.obs_controller.update_vlc_source(
                            VLC_SOURCE_NAME, prep_folder
                        )
                        if success:
                            logger.info("Refreshed VLC playlist for unpause (prepared rotation)")
                            self._initialize_playback_monitor(prep_folder)
                            if self.playback_monitor:
            ...
            # Skip when a prepared rotation is active — the saved position is
            # from live/ content, not the prepared folder.
            if self.current_session_id and not self._prepared_rotation_active:
                session = self.db.get_current_session()
                if session:
                    saved_video = session.get('playback_current_video')
                    saved_cursor = session.get('playback_cursor_ms', 0)
